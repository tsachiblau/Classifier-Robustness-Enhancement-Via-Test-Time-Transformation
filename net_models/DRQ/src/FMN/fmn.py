#Code adjusted from https://github.com/pralab/Fast-Minimum-Norm-FMN-Attack/
...
class FMNAttackLp(MinimizationAttack):
    ...
    def run(
        ...
        for i in range(self.steps):
            ...
            is_adversarial = self.get_is_adversarial_confidence(logits, x_adv, criterion_)
    ...
    def get_is_adversarial_confidence(self, logits, x_adv, criterion):
        if self.confidence is not None:
            logits_, restore_type = ep.astensor_(logits)
            logits_ = logits_.raw
            del logits, x_adv
            current_confidence = torch.softmax(logits_, 1).max(1).values
            confident = current_confidence > self.confidence
            is_adversarial = criterion.labels.raw != logits_.argmax(1)
            is_confident = ep.astensor(confident * is_adversarial)
            return is_confident
        else:
            return criterion(x_adv, logits)
